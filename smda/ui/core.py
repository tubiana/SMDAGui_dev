...
from PyQt5.QtWidgets import (QMainWindow, QMessageBox, QFileDialog, QGraphicsScene, QShortcut)
...
#Tools function
    ...
    def on_actionOpen_triggered(self):
        paramFile, filetype = QFileDialog.getOpenFileName(
                                    self,"Open parameter file",
                                    filter="Parameter file (*.json);; All (*.*)")
        ...
        self.load_param_recurcive(allParameters,itemLoaded,"self.treeWidgetChoosenAnalysis")
    ...
    def load_param_recurcive(self, allParameters, itemLoaded, parentWidget):
        ...
        for param in allParameters:
            if list(param.keys())[0] == "LIGAND":
                ...
                self.load_param_recurcive(param["LIGAND"], itemLoaded,"self.LigandsTreeList")
            else:
                ...
                if param["class"] == self.__class__.__name__:
                    ...
                    item = eval("anlz.{}.{}({}, self)".format(param["class"],
                                                              param["class"],
                                                              parentWidget))
    ...
    def on_actionSave_triggered(self):
            ...
            (paramFile,filetype) = QFileDialog.getSaveFileName(self,
                                    "Save file",
                                    filter="Parameter file (*.json);; All (*.*)")
    ...
    def copyIOParametersToDict(self):
        ...
        self.parameters["outputTrajectoryPath"] = self.lineEditOutputTrajectoryPath.text()
        ...
        self.parameters["format"] = (self.comboBoxOutputFormat.currentIndex(),
                                     self.comboBoxOutputFormat.itemText(self.comboBoxOutputFormat.currentIndex()))
    ...
    def copyIOParametersFromDict(self, dictionnary):
        ...
        if type(dictionnary["inputTrajectoriesPath"]) == type([]):
    ...
    def on_pushButtonAddTrajectory_released(self):
        (file,filetype) = QFileDialog.getOpenFileNames(self,
                                    "Open Trajectory",
    ...
    def on_toolButtonOutputTrajectoryButton_clicked(self):
        (file,filetype) = QFileDialog.getSaveFileName(self,
                                    "Save Trajectory",
                                    filter="Valid Formats (*.arc *.dcd *.binpos *.xtc *.trr *.hdf5 *.h5 *.ncdf *.netcdf \
                                    *.nc *.pdb.gz *.pdb *.lh5 *.crd *.mdcrd *.inpcrd  *.restrt *.rst7 *.ncrst *.lammpstrj \
                                    *.dtr *.stk *.gro *.xyz.gz *.xyz *.tng *.xml *.mol2 *.hoomdxml);;")
    ...
    def add_image(self):
        ...
        img = img.scaled(self.graphicsViewResults.width(),self.graphicsViewResults.height(),
    ...
    def add_analysis_on_TreeWidget(self):
        #1. Top menus
        fluctuationTreeMenu = QtWidgets.QTreeWidgetItem(self.treeWidgetAnalysisAvailable)
        ...
        conservationTreeMenu = QtWidgets.QTreeWidgetItem(self.treeWidgetAnalysisAvailable)
    ...
    def on_treeWidgetAnalysisAvailable_itemSelectionChanged(self):
        ...
        if not 'analysis' in str(type(item)):
    ...
    def on_treeWidgetChoosenAnalysis_itemSelectionChanged(self):
        ...
        if item == None:
            item = self.treeWidgetChoosenAnalysis.setCurrentItem(self.treeWidgetChoosenAnalysis.topLevelItem(0))
        ...
        if self.is_runOK:
            ...
            for i in range(self._numReplica):
                ...
                subtab = self.tabResults[i].findChild(QtWidgets.QTabWidget, "graphicsTabWidget")
    ...
    def on_toolButtonAddAnalysis_clicked(self):
        if self.treeWidgetAnalysisAvailable.hasFocus():
            ...
            if not item:
                return  None
            try:
                newitem=item.__class__(self.treeWidgetChoosenAnalysis, self) #Duplication of this item
    ...
    def check_fill(self):
        ...
        if not self.selectionOK:
            QMessageBox.warning(self, "Error", "An atom selection is not OK. Please search for a red box and correct it")
            return False
    ...
    def check_files_ok(self):
        ...
        if not self.lineEditOutputTrajectoryPath.text() == "":
            if not os.access(os.path.dirname(self.lineEditOutputTrajectoryPath.text()), os.W_OK):
                QMessageBox.warning(self,"Error","I do not have write acces for Output Trajectory")
                return False
            self.saveOutput = True
            
        if not self.lineEditOutputTopologyPath.text() == "":
            if not os.access(os.path.dirname(self.lineEditOutputTopologyPath.text()), os.W_OK):
                QMessageBox.warning(self,"Error","I do not have write access for Output Topology ")
                return False
    ...
    def on_pushButtonRun_clicked(self):
        ...
        if not self.check_fill(): #Check if everything was filed
            return
        if not self.check_files_ok(): #CHeck if files exist and that we have write access
            return
    ...
    def create_output_dirs(self):
        ...
        if self.lineEditOutputFiguresPath.text() == "":
            ...
            folder_name = '.'.join(os.path.basename(os.path.realpath(topol_location)).split('.')[:-1])
    ...
    def do_item_analysis(self, root, traj, replica=0):
        ...
        for i in range(childCount):
            ...
            if (item.__class__.__name__ == "HBonds") and ONTESTING == True:
    ...
    def returnAtomsIndexToRemove(self):
        ...
        try:
            firstFrame = md.load_frame(self.listWidgetInputTrajectories.item(0).text(),
                        self.lineEditInputTopologyPath.text(), index=0)
        except:
            return False
        ...
        if self.lineEditInputStripping.text() != "":
            hasAtomToRemove = True
            removeIndexes.append(firstFrame.top.select(f"not {self.lineEditInputStripping.text()}"))
    ...
    def read_single_traj_from_listWidget(self, index=0, frame=None):
        ...
        if atomsToRemove not in [None, False]:
            traj = md.load(self.listWidgetInputTrajectories.item(index).text(),
                           top=self.lineEditInputTopologyPath.text(),
                           stride=self.spinBoxInputDT.value(),
                           atom_indices=atomsToRemove,
                           frame=frame)
    ...
    def read_and_return_traj(self):
        ...
        if count > 1:
            ...
            if self.checkBoxReplicas.isChecked():
                ...
                #Rework timestep
                try:
                    traj.time = np.arange(traj.time[0],
                                          len(traj)*traj.timestep,
                                          traj.timestep)
                except:
    ...
    def start_Analysis(self):
        ...
        self.on_treeWidgetChoosenAnalysis_itemSelectionChanged()
        self.statusbar.showMessage("Done! Click on an analysis to see results or check IMG folder.")
...
#        HBONDS LIG-POCKET
        ...
        rmsdLigOnProt = anlz.RMSD.RMSD().__class__(self.LigandsTreeList, self) #Duplication of this item
        ...
        #RMSF on LIGAND
        rmsfLigand = anlz.RMSF.RMSF().__class__(self.LigandsTreeList, self)  # Duplication of this item
        ...
        #RMSF on pocket
        rmsfPocket = anlz.RMSF.RMSF().__class__(self.LigandsTreeList, self)  # Duplication of this item
        ...
        SASAPocket = anlz.Surface.Surface().__class__(self.LigandsTreeList, self)  # Duplication of this item
        ...
        #HBONDS
        Hbonds = anlz.HBonds.HBonds().__class__(self.LigandsTreeList, self)  # Duplication of this item
        Hbonds.lineEditName.setText("LIGAND -- pocket")
        Hbonds.lineEditSelection1.setText(f"protein")
